...
class AddImage {
  ...
  Future getChatImage(String username,String fileName)async{
    ...
    try {
      String url =
          await storage.ref().child('chat_image/$username/$fileName').getDownloadURL();
      return url;
  ...
  Future getProfileImage(String username) async {
    ...
    try {
      ...
      await db.addImageUrlToUser(username, url);
      //await db.updateImageUrlInChatRoom(username,url);
      print(url);
      return url;
